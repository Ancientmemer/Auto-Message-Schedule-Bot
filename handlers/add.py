...
def register_add_handlers(app):
    ...
    async def add(_, message):
        ...
        buttons = [[InlineKeyboardButton(k, callback_data=f"set_{v}") ] for k,v in TIMES.items()]
        await message.reply("⏱ Select time", reply_markup=InlineKeyboardMarkup(buttons))
    ...
    async def set_time(_, cq):
        ...
        await cq.message.edit("✅ Auto message started")
